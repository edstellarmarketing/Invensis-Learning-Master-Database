    ...
    >
      ...
      >
        <div className="band-gradient-soft flex items-center justify-between rounded-t-2xl border-b px-6 py-4">
          <h2 className="text-lg font-bold">Manage courses &amp; categories</h2>
        ...
        >
          ...
          <div className="mb-4 flex items-end gap-2">
            ...
            <button onClick={addCategory} disabled={busy || !newCat.trim()} className="inline-flex items-center gap-1.5 rounded-md bg-primary px-3 py-1.5 text-sm font-medium text-primary-contrast disabled:opacity-60">
            ...
            {categories.map((cat) => (
              <div key={cat.slug} className="rounded-lg border">
                <div className="flex items-center gap-2 border-b bg-surface-2/40 px-3 py-2">
                  ...
                  <button onClick={() => renameCategory(cat.slug, cat.name)} disabled={busy} aria-label="Rename category" className="p-1 text-text-muted hover:text-primary"><Pencil size={13} /></button>
                  <button onClick={() => removeCategory(cat.slug, cat.name, cat.courses.length)} disabled={busy} aria-label="Delete category" className="p-1 text-text-muted hover:text-red-500"><Trash2 size={13} /></button>
                </div>
                  ...
                  {cat.courses.map((co) => (
                    <li key={co.slug} className="flex items-center gap-2 px-3 py-1.5">
                      <button
                        ...
                        className={co.featured ? "text-warning" : "text-text-muted hover:text-warning"}
                      >
                        <Star size={14} fill={co.featured ? "currentColor" : "none"} />
                      </button>
                      <span className="flex-1 text-sm">{co.name}</span>
                      <button onClick={() => renameCourse(co.slug, co.name)} disabled={busy} aria-label="Rename course" className="p-1 text-text-muted hover:text-primary"><Pencil size={13} /></button>
                      <button onClick={() => removeCourse(co.slug, co.name)} disabled={busy} aria-label="Delete course" className="p-1 text-text-muted hover:text-red-500"><Trash2 size={13} /></button>
                    </li>
                  ))}
                </ul>
                <div className="border-t px-3 py-2">
                  {addCourseFor === cat.slug ? (
                    <div className="flex flex-wrap items-end gap-2">
                      <input className={`${field} min-w-[160px] flex-1`} value={courseName} onChange={(e) => setCourseName(e.target.value)} placeholder="Course name" autoFocus />
                      ...
                      <button onClick={() => addCourseSubmit(cat.slug)} disabled={busy || !courseName.trim()} className="rounded-md bg-primary px-3 py-1.5 text-sm font-medium text-primary-contrast disabled:opacity-60">Add</button>
                      <button onClick={() => setAddCourseFor(null)} className="rounded-md border px-3 py-1.5 text-sm hover:bg-surface-2">Cancel</button>
                    </div>
                  ) : (
                    <button onClick={() => { setAddCourseFor(cat.slug); setCourseName(""); setCourseFeatured(false); }} className="inline-flex items-center gap-1 text-sm text-primary hover:underline">
